...
import {MapBoxComponent} from "@bim/src/MapBoxComponent";
...
export class CullerRenderer {
   ...
   */
  readonly onViewUpdated: OBC.Event<any> | OBC.AsyncEvent<any> =
    new OBC.AsyncEvent<any>();
  ...
  updateVisibility = async (force?: boolean) => {
    ...
    const isMap = mapBoxSignal.value;
    const mapCamera = this.components.get(MapBoxComponent).camera;

    const camera = isMap ? mapCamera : this.world.camera.three;
    camera.updateMatrix();
